...
def get_earliest_date(mediafile):
    exif_dates = subprocess.check_output(["/usr/local/bin/exiftool", "-G1", "-Alldates", "-j", mediafile])
...
if __name__ == '__main__':
    ...
    for root, dirs, files in os.walk(args.targetfolder, topdown=True):
        for name in files:
            ...
            subprocess.run(
                ["/usr/local/bin/exiftool", "-q", "-m", "-overwrite_original", f"-{dto_key}='{mindate}'", mediafile])
